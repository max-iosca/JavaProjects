	...
	public void riconsegnaLibro(Cliente cliente, Libro libro) {
		...
		try {
			...
			for (Libro item : libri) {
				...
				if (item == null) {
					cliente.riconsegnaLibro(libro); // qui puó scatenarsi una eccezione
			...
			// se il libro non é posseduto dal cliente salta direttamente qui
	...
	public void affittaLibro(Cliente cliente, Libro libro) {
		...
		if (!trovato) {
			...
			try {
				...
				System.out.println("Non c'è spazio libero nello zaino del cliente. \n");
